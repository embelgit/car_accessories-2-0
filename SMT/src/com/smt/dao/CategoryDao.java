...
public class CategoryDao {
...

	{
		...
		try{
		 ...
		 list = query.list();
		 System.out.println("in getAllMainCategories() list size - "+query.list().size());
		}
			catch(RuntimeException e){	
...

	{
		...
		try{
		 hbu = HibernateUtility.getInstance();
		 session = hbu.getHibernateSession();
		 Query query = session.createQuery("from GoodsReceiveBarrelHibernate");
		 list = query.list();
		 System.out.println("in getAllMainCategories()--------------------------------- list size - "+query.list().size());
		}
			catch(RuntimeException e){	
	...
	public List getSubCategoriesByRootcategory(String mainCatId)
	{
		...
		List list=null;
			try
			{
				hbu = HibernateUtility.getInstance();
				session = hbu.getHibernateSession();
				Query query = session.createQuery("select pkSubcatId , subcatName from SubCategory sc where  sc.fkRootcatId ="+mainCatId);
				list = query.list();
			}
				catch(RuntimeException e)
				{	
