...
public class EmployeePaymentDao {
	...
	public List<EmployeePaymentDetail> getEmployeePaymentDetailsDateWise(
			String fDate, String tDate, String fkRootempId) {
		...
		try
		{
			...
			hbu = HibernateUtility.getInstance();
		 session = hbu.getHibernateSession();
		 Query query2 = session.createSQLQuery("select first_name, last_name, payment, payment_mode, insert_date, person_name, paymentType,reason FROM employee_details RIGHT JOIN employee_payment ON employee_details.pk_empoyee_id = employee_payment.fk_employee_id WHERE fk_employee_id =:empId AND insert_date BETWEEN '"+fDate+"' AND '"+tDate+"'");
