...
String gstTinNo = (String) session.getAttribute("gstTinNo"); 
	...
	try {
	 ...
	 // step 3
		...
		//ResultSet rs = stmt.executeQuery("select ItemName, CategoryName, Quantity, SalePrice, ContactNo, OwnerName, TotalAmount, Discount, GrossTotal, Date, totalperitem, TaxAmount,Gst,Igst from otherbill where BillNo =" + billno);
		...
		String mono = rs.getString("contact_no");
 ...
 */
	...
	document.add(infotable1);
		...
		float[] columnWidths = {0.3f,0.5f,0.6f,0.6f, 0.4f,0.4f,0.5f,0.4f };
...
//String gstTinNo = (String) session.getAttribute("gstTinNo"); 
	...
	try {
	 ...
	 // step 3
		...
		float[] infoColumnWidths = { 0.3f,0.4f};
		infotable.setWidths(infoColumnWidths);
	...
	//////////////km///////	
		...
		/* PdfPTable infotable21 = new PdfPTable(1);
		infotable21.setWidthPercentage(100);

		float[] infoColumnWidths21 = { 0.3f};
	...
	//table for particulars
		...
		//footer of service spares
			...
			table_cell13 = new PdfPCell(new Phrase("Description:" ));
			table_cell13.setHorizontalAlignment(Element.ALIGN_LEFT);
			//table_cell5.setBorder(Rectangle.RIGHT|Rectangle.TOP);
			table13.addCell(table_cell13);
