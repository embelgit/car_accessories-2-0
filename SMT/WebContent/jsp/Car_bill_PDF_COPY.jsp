...
<%@page import="com.lowagie.tools.Executable"%>
...
<%
	...
	try {
		...
		ResultSet rs = stmt.executeQuery("select CarNo, ItemName, CategoryName, Quantity, SalePrice, ContactNo, OwnerName, TotalAmount, Discount, GrossTotal, Date , totalperitem, TaxAmount, LaberCharges from customerbill where BillNo =" + billno);
...
while (rs.next()) {
            ...
            laberCharges = rs.getDouble("LaberCharges");
			
			itemCount++;
			finalDis = finalDis + finalDiscountAmt;
			finalLaberCharges = finalLaberCharges + laberCharges;
			finalgross = (finalTotAmountWithoutDis - finalDis) + finalLaberCharges ;
		...
		table_cell = new PdfPCell(new Phrase("Laber Charges :", font14));
		table_cell.setPaddingBottom(1);
		table_cell.setColspan(3);
		table_cell.setHorizontalAlignment(Element.ALIGN_RIGHT);
		table_cell.setBorder(Rectangle.NO_BORDER);
		...
		String laber;
		if(finalLaberCharges > 0){
			laber = df.format(finalLaberCharges);
		}else{
			laber = "0.00";
		}
		
		table_cell = new PdfPCell(new Phrase(laber, font14));
		table_cell.setHorizontalAlignment(Element.ALIGN_RIGHT);
		table_cell.setBorder(Rectangle.NO_BORDER);
		table_cell.setPaddingBottom(1);
		table_cell.setPaddingTop(1);
