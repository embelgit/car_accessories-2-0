	 ...
	 for (i = 0; i < list.options.length; ++i) {
		...
		$.post('/SMT/jsp/utility/controller.jsp',params,function(data)
		    	{  
			           ...
			           window.open("CreditCustomerBillPDFNew.jsp");
...
function getproductgrid(){
	...
	$.post('/SMT/jsp/utility/controller.jsp',params,function(data)
		...
		$("#list4").jqGrid({
			...
			colNames:['pk_temp_id','item_id','BarcodeNO','CatName','ItemName','HSN/SAC','Quantity','SalePrice',"Total SP","SPExTax",'GST%','IGST%','Tax Amt','Discount','DiscountAmt','Total Amt','Stock'],
			colModel:[ 
            ...
            'cellEdit':true,
	           afterSaveCell: function () {
            	...
            	if(+quantity > +stock){
            		var aa = "0";
            		alert("Stock not available");
    				$("#list4").jqGrid("setCell",rowId, "quantity", aa);
    				return false;
            	}
            	
            	//tota = quantity * salePrice;
            	totAmt = quantity * salePrice;
            	$("#list4").jqGrid("setCell", rowId, "TotalQuan", totAmt);
            	/*var totalIncDisc= (tota*(discount/100));
            	var finalTotal= totAmt-totalIncDisc;
            	$("#list4").jqGrid("setCell", rowId, "discountAmt", totalIncDisc);
             */
            	...
            	$("#list4").jqGrid("setCell", rowId, "buyPriceExTax", totalWithVatAmtTot);
            	$("#list4").jqGrid("setCell", rowId, "discountAmt", discount1);
            	$("#list4").jqGrid("setCell", rowId, "taxAmount", taxAmount);
             	$("#list4").jqGrid("setCell", rowId, "total",  totAmt);
             	var Total = 0;
            	var count = jQuery("#list4").jqGrid('getGridParam', 'records');
				var allRowsInGrid1 = $('#list4').getGridParam('data');
				var AllRows=JSON.stringify(allRowsInGrid1);
		 ...
		 $('#list4').navGrid('#jqGridPager',
	                ...
	                
	                {
	                     ...
	                     */
	                    	...
	                    	$("#list4").jqGrid("setCell", rowId, "buyPriceExTax", BPExTax);
	                    	$("#list4").jqGrid("setCell", rowId, "discountAmt", discount1);
	                    	$("#list4").jqGrid("setCell", rowId, "taxAmount", taxAmount);
	                     	$("#list4").jqGrid("setCell", rowId, "total",  finaltota);
	                     	var Total = 0;
	                    	var count = jQuery("#list4").jqGrid('getGridParam', 'records');
	        				var allRowsInGrid1 = $('#list4').getGridParam('data');
	        				var AllRows=JSON.stringify(allRowsInGrid1);
           ...
           // rowheight: 300,
             ...
             */
            	...
            	if(Total1 != undefined){
                    ...
                    document.getElementById("grossTotalservice").value = Math.round(Total);
		...
		$("#listOil").jqGrid({
           ...
           // rowheight: 300,
            ...
            'cellEdit':true,
	           afterSaveCell: function () {
            	...
            	if(+quantity > +litre){
    				...
    				$("#listOil").jqGrid("setCell",rowId, "quantitydouble", ab);
    				return false;
            	}
            	
            	//tota = quantity * salePrice;
            	totAmt = quantity * salePrice;
            	$("#listOil").jqGrid("setCell", rowId, "TotalQuan", totAmt);
            	/*var totalIncDisc= (tota*(discount/100));
            	var finalTotal= totAmt-totalIncDisc;
             ...
             */
            	...
            	$("#listOil").jqGrid("setCell", rowId, "buyPriceExTax", BPExTax);
            	$("#listOil").jqGrid("setCell", rowId, "discountAmt", discount1);
            	$("#listOil").jqGrid("setCell", rowId, "taxAmt", taxAmount);
            	...
            	if(Total1 != undefined){
             	   ...
             	   var spareGrossTotal = document.getElementById("grossTotal").value;
             	   var grossTotalservice = document.getElementById("grossTotalservice").value;
                   var grossTotalOil = document.getElementById("grossTotalOil").value;
                   GrandGrossTotalOil =Number(spareGrossTotal) + Number(grossTotalservice) + Number(grossTotalOil);
...
function resBillService() {
	...
	var params= {};
	   ...
	   var totalAmount=$('#totalAmount').val();
	   var carID=$('#carID').val();
	    var grossTotal=$('#grossTotalservice').val();
	   var wholeTotal=$('#wholeTotal').val();
		...
		params["methodName"] = "registerBillServicecredit";
		
		$.post('/SMT/jsp/utility/controller.jsp', params,
				function(data) {
					alert(data);
					//window.open("Car_bill_PDF.jsp");
//new					window.open("BillingPdfNew.jsp");
					location.reload(true);
...
function resOtherBill(){
	...
	for (var i = 0; i < count; i++) {
		...
		*/
	    ...
	    params["methodName"] = "registerOtherBillcredit";
