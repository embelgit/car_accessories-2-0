  function forOptionselect()
 		...
 		var ac2=0;
 		document.getElementById("card").value = ac2;  
 		var ac1 =  document.getElementById("resolution").value;
 		
 		document.getElementById("chequeAmt").value = ac1;
 		 document.custord.chequeAmt.disabled = true;
 		  } 
	     }
...
params["cash"] = cash;
	...
	params["chequeNo"] = chequeNo;
	params["chequeAmt"] = chequeAmt;
		...
		$.post('/SMT/jsp/utility/controller.jsp',params,function(data)
 	    	{
				...
				var sss = $.map(jsonData, function(el) { return el.itemName });
				document.getElementById("lastScanItemName").value=sss;
				
				var aaa = $.map(jsonData, function(el) { return el.quantity });
				document.getElementById("lastScanQuantity").value=aaa;
				
				var xxx = $.map(jsonData, function(el) { return el.salePrice });
				document.getElementById("lastScanSalePrice").value=xxx;
		        ...
		        function sumFormatter (cellvalue, options, rowObject)
		        {
		        	...
		        	var jam=0;
		        	var quan= options.rowData.quantity; 
		        	var shree = document.custord.quanRes.value;
		        	...
		        	for (var i = 0; i < count; i++) {
		            	...
		            	jam = jam + quantity;
		        		 ...
		        		 document.getElementById("quanRes").value = jam;
